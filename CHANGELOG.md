...
- **Release build exceeding CI timeouts.** The `release` profile used fat LTO (`lto = true`) with `codegen-units = 1`, serializing codegen of the ~500-crate dependency graph (ort, nokhwa, windows…) past the 120-minute runner budget. The first attempt switched to `lto = "thin"` + `codegen-units = 16` — dependency crates then compiled in under 5 minutes, but the **final LTO pass + link of the binaries** still took >1h49m on the 2-vCPU runner (log stalls right after the last `Compiling …` line with zero output until the job is killed). Final fix: **LTO off** (`lto = false`) + `opt-level = 2` + `codegen-units = 16`. This removes the cross-crate link-time optimization entirely, so the final codegen+link is fast and the build fits in the CI budget. The heavy compute (ONNX inference) runs inside the prebuilt native `ort` runtime, so Rust-side optimization loss is negligible for this utility app. The workflow also gained a `concurrency` guard so re-pushed tags cancel in-flight builds instead of doubling runner time.
