...
export default function DisplaySettings({
  ...
  onBrightnessChange,
  onHdrChange,
  onAiBrightnessChange,
  onAiBrightnessConfigChange,
  onRefreshRateChange,
  ...
  return (
    <div className="card">
      ...
      <div className="slider-row" style={{ marginBottom: 20 }}>
        ...
        <input
          ...
          onMouseUp={() => {
            ...
            void handleAsync('Brightness', () => onBrightnessChange(brightness));
          }}
          onTouchEnd={() => {
            isDragging.current = false;
            void handleAsync('Brightness', () => onBrightnessChange(brightness));
      ...
      <ToggleRow
        label={t('display.hdr')}
        desc={
          capabilities != null && !capabilities.has_igcl
            ? t('display.igclNotAvailable')
            : display.hdr_enabled
              ? t('display.hdrEnabled')
              : t('display.hdrDisabled')
        }
      ...
      <ToggleRow
        ...
        desc={
          capabilities != null && !capabilities.has_igcl ? t('display.igclNotAvailable') : undefined
        }
          ...
          {sensitivityOpen && (
            ...
            >
              ...
              <div className="slider-row" style={{ marginBottom: 12 }}>
                <span className="slider-label" style={{ fontSize: 12 }}>
                  {t('display.sensitivityMin')}
                </span>
                <input
                  type="range"
                  min={5}
                  ...
                  onChange={(e) =>
                    setLocalCfg({ ...editCfg, min_brightness: Number(e.target.value) })
                  }
                />
                <span className="slider-value" style={{ fontSize: 12 }}>
                  {editCfg.min_brightness}%
                </span>
              </div>
              ...
              <div className="slider-row" style={{ marginBottom: 12 }}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setLocalCfg({ ...editCfg, max_brightness: Number(e.target.value) })
                  }
                />
                <span className="slider-value" style={{ fontSize: 12 }}>
                  {editCfg.max_brightness}%
                </span>
              </div>
              ...
              <div className="slider-row" style={{ marginBottom: 4 }}>
                <span className="slider-label" style={{ fontSize: 12 }}>
                  {t('display.sensitivitySmoothing')}
                </span>
                <input
                  type="range"
                  min={0}
              ...
              >
                <div style={{ fontWeight: 600, marginBottom: 4 }}>
                  ℹ {t('display.sensitivityHowItWorks')}
                </div>
                <div style={{ marginBottom: 2 }}>· {t('display.sensitivityHelpMin')}</div>
                <div style={{ marginBottom: 2 }}>
                  ·{' '}
                  {t('display.sensitivityHelpLevel', {
                    lux: String(Math.round(200000 / editCfg.sensitivity)),
                  })}
                </div>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    const newCfg: AiBrightnessConfig = {
                      enabled: display.ai_brightness,
                      ...defaults,
                    };
      ...
      {display.available_refresh_rates && display.available_refresh_rates.length > 1 ? (
            ...
            {display.available_refresh_rates.map((hz) => {
              ...
              return (
                <button
                  ...
                  title={
                    isMax && hz >= 90
                      ? `${hz} Hz — Windows 11 Dynamic Refresh Rate activates automatically at max Hz (switches 60\u2194${hz} Hz based on content)`
                      : `${hz} Hz`
                  }
                  onClick={() => {
                    if (!isActive)
                      void handleAsync(
                        'Refresh rate',
                        () => onRefreshRateChange?.(hz) ?? Promise.resolve(),
                      );
                  }}
                  style={{ minWidth: 52 }}
                >
        ...
        <div className="stat-row" style={{ marginTop: 8 }}>
          ...
          <span className="stat-value">
            {display.refresh_rate_hz} {t('display.hz')}
          </span>
        </div>
      )}
      {display.dynamic_refresh_rate_capable && (
        <div
          style={{ fontSize: 11, color: 'var(--color-text-dim)', marginTop: 4, lineHeight: 1.4 }}
        >
          ★ Dynamic Refresh Rate active — Windows 11 automatically switches to lower Hz to save
          power when high frame rate isn’t needed.
        </div>
      )}
