...
export default function WiFiManager() {
  ...
  return (
    <div className="card">
      ...
      {/* Current connection */}
      {status?.connected && (
        <div style={{ marginTop: 12, padding: 10, background: "var(--bg-hover)", borderRadius: "var(--r-xs)" }}>
        ...
        {networks.length === 0 ? (
          ...
          networks.map((n) => (
            <div
              key={n.ssid}
              className="stat-row"
              style={{ padding: "8px 10px", marginBottom: 4, borderRadius: "var(--r-xs)", cursor: "pointer" }}
              onClick={() => setSelectedSsid(selectedSsid === n.ssid ? null : n.ssid)}
            >
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, fontWeight: 500 }}>
                  {n.ssid}
                  {n.connected && <span style={{ color: "var(--success)", marginLeft: 8 }}>● {t("wifi.connected")}</span>}
                </div>
              ...
              {selectedSsid === n.ssid && !n.connected && (
                <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                  ...
                  <button
                    ...
                    onClick={(e) => { e.stopPropagation(); handleConnect(n.ssid); }}
                    disabled={connecting}
      ...
      <button className="btn btn-secondary" onClick={handleRefresh} style={{ marginTop: 12, width: "100%" }}>
        🔄 {t("wifi.refresh")}
      </button>
    </div>
  );
