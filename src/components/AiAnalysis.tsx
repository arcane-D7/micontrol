    ...
    >
      ...
      {ticks.map((tick, i) => (
        <g key={i}>
          ...
          />
          <text x={PAD.left - 4} y={toY(tick) + 3.5} textAnchor="end"
    ...
    <div style={{ display: "flex", flexWrap: "wrap", gap: "6px 14px", marginTop: 6 }}>
      {items.map((item) => (
        <div key={item.label} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, color: "var(--text-muted)" }}>
          <span style={{ width: 10, height: 2.5, background: item.color, borderRadius: 2, display: "inline-block", flexShrink: 0 }} />
          {item.label}
          {item.last !== undefined && (
...
function ProcessBars({ processes }: { processes: Array<{ name: string; cpu_pct: number; memory_mb: number }> }) {
  if (!processes.length) {
    return <div style={{ fontSize: 12, color: "var(--text-dim)" }}>{t("aiAnalysis.charts.noData")}</div>;
  }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
      {processes.slice(0, 8).map((p) => (
        <div key={p.name} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 11 }}>
          <span style={{
            width: 140, flexShrink: 0, overflow: "hidden", textOverflow: "ellipsis",
          ...
          <span style={{ width: 42, textAlign: "right", fontFamily: "var(--font-mono)", color: "var(--text-dim)" }}>
            {p.cpu_pct.toFixed(1)}%
          </span>
          <span style={{ width: 54, textAlign: "right", fontFamily: "var(--font-mono)", color: "var(--text-dim)", fontSize: 10 }}>
            {p.memory_mb >= 1024
...
function nextAnalysisIn(dailyAnalyses: number): string {
  try {
    const sched = JSON.parse(localStorage.getItem("micontrol_analysis_schedule_v1") ?? '{"recent":[]}') as { recent: string[] };
...
export default function AiAnalysis({ hw, ai, onOpenSettings }: Props) {
  ...
  async function handleAnalyze() {
    if (!ai.isConfigured) { onOpenSettings(); return; }
    setAnalyzing(true);
    ...
    try {
      ...
      const text = await ai.analyzeWithLogs(currentLogs, {
      ...
      const displayMsg =
        msg === "api_key_missing" ? t("settings.apiKeyMissing")
        : msg === "no_logs" ? t("aiAnalysis.analysis.notEnoughData")
        : msg;
  ...
  const cpuTemps: SeriesDef = { values: chartLogs.map((l) => l.cpu_temp), color: "var(--error, #f44336)", label: t("aiAnalysis.charts.cpuTemp") };
  const gpuTemps: SeriesDef = { values: chartLogs.map((l) => l.gpu_temp), color: "#ff9800", label: t("aiAnalysis.charts.gpuTemp") };
  const tdpSeries: SeriesDef = { values: chartLogs.filter((l) => l.tdp_watts != null).map((l) => l.tdp_watts as number), color: "var(--info, #2196f3)", label: t("aiAnalysis.charts.tdp") };
  const cpuPct: SeriesDef = { values: chartLogs.map((l) => l.cpu_pct), color: "var(--accent)", label: "CPU%" };
  const gpuPct: SeriesDef = { values: chartLogs.map((l) => l.gpu_pct), color: "#00bcd4", label: "GPU%" };

  const lastLog = logs[logs.length - 1];
  const topProcs = lastLog?.top_processes ?? [];

  const spanMs = chartLogs.length >= 2
    ...
    <>
      ...
      <div className="card">
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
          <div>
            <div className="card-title" style={{ marginBottom: 2 }}>{t("aiAnalysis.settings.title")}</div>
            <div style={{ fontSize: 12, color: "var(--text-muted)" }}>{t("aiAnalysis.settings.subtitle")}</div>
          </div>
          <label className="toggle-switch">
            <input
              ...
              onChange={(e) => {
                ...
                addToast(
                  e.target.checked ? t("aiAnalysis.settings.toastEnabled") : t("aiAnalysis.settings.toastDisabled"),
                  "info"
                );
              }}
            />
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 2 }}>{t("aiAnalysis.settings.pollIntervalDesc")}</div>
              </div>
              <select
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 2 }}>{t("aiAnalysis.settings.dailyAnalysesDesc")}</div>
              </div>
              <select
                className="select-input"
                style={{ minWidth: 130 }}
            ...
            {/* Model hint */}
            {!ai.isConfigured ? (
              <div style={{ fontSize: 12, color: "var(--warning, #ff9800)", display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <div style={{ fontSize: 11, color: "var(--text-dim)" }}>
                🤖 {t("aiAnalysis.settings.modelHint", { model: ai.settings.openai_model || "gpt-4o-mini" })}
              </div>
            )}

            {/* Stats row */}
            <div style={{ display: "flex", gap: 16, paddingTop: 6, borderTop: "1px solid var(--border)", flexWrap: "wrap" }}>
              <div style={{ fontSize: 11, color: "var(--text-muted)" }}>
                {t("aiAnalysis.settings.logCount")}: <strong style={{ color: "var(--text)" }}>{logs.length}</strong> / {500}
              </div>
              {spanLabel && (
                <div style={{ fontSize: 11, color: "var(--text-muted)" }}>
                  {t("aiAnalysis.settings.span")}: <strong style={{ color: "var(--text)" }}>{spanLabel}</strong>
                </div>
              )}
              <button
      ...
      {/* ── Charts card ───────────────────────────────────────────────────── */}
      <div className="card">
        <div className="card-title" style={{ marginBottom: 2 }}>{t("aiAnalysis.charts.title")}</div>
        <div style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 14 }}>
          {chartLogs.length > 0
            ? t("aiAnalysis.charts.subtitle", { n: String(chartLogs.length), span: spanLabel || "—" })
            : t("aiAnalysis.charts.noData")}
        </div>
        ...
        {chartLogs.length >= 2 ? (
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            {/* Temperature chart */}
            <div>
              <div style={{ fontSize: 11, fontWeight: 600, color: "var(--text-dim)", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 6 }}>
                {t("aiAnalysis.charts.temperature")} (°C)
              </div>
              <LineChart series={[cpuTemps, gpuTemps]} height={100} unit="°" />
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: "var(--text-dim)", marginTop: 2, paddingInline: 36 }}>
                <span>{fmtTime(chartLogs[0].ts)}</span>
                <span>{fmtTime(chartLogs[chartLogs.length - 1].ts)}</span>
              </div>
              <ChartLegend items={[
                { color: cpuTemps.color, label: cpuTemps.label, last: `${lastLog?.cpu_temp.toFixed(0)}°C` },
                { color: gpuTemps.color, label: gpuTemps.label, last: `${lastLog?.gpu_temp.toFixed(0)}°C` },
              ]} />
            </div>
            ...
            {tdpSeries.values.length >= 2 && (
              <div>
                ...
                <ChartLegend items={[
                  { color: tdpSeries.color, label: tdpSeries.label, last: `${lastLog?.tdp_watts?.toFixed(1) ?? "—"} W` },
                ]} />
              </div>
            )}
            ...
            <div>
              ...
              <ChartLegend items={[
                { color: cpuPct.color, label: cpuPct.label, last: `${lastLog?.cpu_pct.toFixed(0)}%` },
                { color: gpuPct.color, label: gpuPct.label, last: `${lastLog?.gpu_pct.toFixed(0)}%` },
              ]} />
            </div>
          ...
          <div style={{
            ...
            padding: "32px 0", gap: 8, color: "var(--text-dim)",
          }}>
      ...
      <div className="card">
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, marginBottom: 12 }}>
          <div>
            <div className="card-title" style={{ marginBottom: 2 }}>{t("aiAnalysis.analysis.title")}</div>
            <div style={{ fontSize: 12, color: "var(--text-muted)" }}>
              {lastAnalysis
                ? t("aiAnalysis.analysis.lastAt", { time: fmtDatetime(lastAnalysis.ts), n: String(lastAnalysis.log_count) })
        ...
        {analyzeError && (
          <div style={{
            padding: "10px 14px", background: "color-mix(in srgb, var(--error, #f44336) 12%, transparent)",
            borderRadius: "var(--r-sm)", borderLeft: "3px solid var(--error, #f44336)",
            fontSize: 12, color: "var(--error, #f44336)", marginBottom: 12,
          }}>
            {analyzeError}
          </div>
        )}
        ...
        {!lastAnalysis && !analyzing && !analyzeError && (
          <div style={{
            display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
            padding: "24px 0", gap: 8, color: "var(--text-dim)",
          }}>
      ...
      <div className="card">
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <div className="card-title" style={{ marginBottom: 2 }}>{t("aiAnalysis.logs.title")}</div>
            <div style={{ fontSize: 12, color: "var(--text-muted)" }}>
              {logs.length} {t("aiAnalysis.logs.entries")}
            </div>
          </div>
          <button className="btn-secondary" style={{ fontSize: 12 }} onClick={() => setShowLogTable((v) => !v)}>
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                    <thead>
                      <tr style={{ color: "var(--text-muted)", borderBottom: "1px solid var(--border)" }}>
                        <th style={{ textAlign: "left", padding: "4px 6px", fontWeight: 500 }}>{t("aiAnalysis.logs.time")}</th>
                        <th style={{ textAlign: "left", padding: "4px 6px", fontWeight: 500 }}>{t("aiAnalysis.logs.mode")}</th>
                        <th style={{ textAlign: "right", padding: "4px 6px", fontWeight: 500 }}>CPU°C</th>
                        <th style={{ textAlign: "right", padding: "4px 6px", fontWeight: 500 }}>GPU°C</th>
                        <th style={{ textAlign: "right", padding: "4px 6px", fontWeight: 500 }}>TDP W</th>
                        <th style={{ textAlign: "right", padding: "4px 6px", fontWeight: 500 }}>CPU%</th>
                        <th style={{ textAlign: "right", padding: "4px 6px", fontWeight: 500 }}>GPU%</th>
                        <th style={{ textAlign: "right", padding: "4px 6px", fontWeight: 500 }}>🔋%</th>
                      </tr>
                    </thead>
                    <tbody>
                      {logs.slice(-logTableCount).reverse().map((e, i) => (
                        <tr key={i} style={{ borderBottom: "1px solid var(--border-faint, var(--border))" }}>
                          <td style={{ padding: "3px 6px", fontFamily: "var(--font-mono)", color: "var(--text-dim)" }}>{fmtTime(e.ts)}</td>
                          <td style={{ padding: "3px 6px", color: "var(--accent)" }}>{e.mode}</td>
                          <td style={{ padding: "3px 6px", textAlign: "right", fontFamily: "var(--font-mono)" }}>{e.cpu_temp.toFixed(0)}</td>
                          <td style={{ padding: "3px 6px", textAlign: "right", fontFamily: "var(--font-mono)" }}>{e.gpu_temp.toFixed(0)}</td>
                          <td style={{ padding: "3px 6px", textAlign: "right", fontFamily: "var(--font-mono)" }}>{e.tdp_watts != null ? e.tdp_watts.toFixed(1) : "—"}</td>
                          <td style={{ padding: "3px 6px", textAlign: "right", fontFamily: "var(--font-mono)" }}>{e.cpu_pct.toFixed(0)}</td>
                          <td style={{ padding: "3px 6px", textAlign: "right", fontFamily: "var(--font-mono)" }}>{e.gpu_pct.toFixed(0)}</td>
                          <td style={{ padding: "3px 6px", textAlign: "right", fontFamily: "var(--font-mono)" }}>{e.battery_level != null ? e.battery_level.toFixed(0) : "—"}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
