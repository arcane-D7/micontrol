...
pub fn get_autostart() -> Result<bool> {
    #[cfg(windows)]
    {
        ...
        unsafe {
            let key_w: Vec<u16> = OsStr::new(STARTUP_REG_KEY).encode_wide().chain(Some(0)).collect();
            let mut hkey = std::mem::zeroed();
            if RegOpenKeyExW(
                HKEY_CURRENT_USER, PCWSTR(key_w.as_ptr()), 0,
                windows::Win32::System::Registry::KEY_READ, &mut hkey,
            ).is_err() {
                return Ok(false);
            }
            let val_w: Vec<u16> = OsStr::new(STARTUP_REG_VALUE).encode_wide().chain(Some(0)).collect();
            let mut size = 512u32;
            let mut ty = REG_VALUE_TYPE::default();
            let exists = RegQueryValueExW(
                hkey, PCWSTR(val_w.as_ptr()), None, Some(&mut ty), None, Some(&mut size),
            ).is_ok();
...
pub fn set_autostart(enabled: bool) -> Result<()> {
    #[cfg(windows)]
    {
        ...
        unsafe {
            ...
            let mut hkey = std::mem::zeroed();
            RegOpenKeyExW(
                HKEY_CURRENT_USER, PCWSTR(key_w.as_ptr()), 0, KEY_WRITE, &mut hkey,
            ).ok().context("Open startup registry key")?;
            ...
            if enabled {
                ...
                RegSetValueExW(
                    hkey, PCWSTR(val_w.as_ptr()), 0, REG_SZ, Some(bytes),
                ).ok().context("Write startup entry")?;
            } else {
